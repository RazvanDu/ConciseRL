...
def train(args):
    ...
    print("WAITING?")
    ray.get(refs)
    # ready_refs, remaining_refs = ray.wait(refs, num_returns=len(refs), timeout=5)

    # print(f"Finished {ready_refs} out of {refs} tasks")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scheduler_type", type=str, default="input", help="Type of scheduler[cosine/warmup_with_constant_lr]")
    ...
    # Save args as a json file in the checkpoint directory

    print("GOT HERE!!")

    os.makedirs(args.ckpt_path, exist_ok=True)
    with open(os.path.join(args.ckpt_path, "train_config.json"), "w") as f:
